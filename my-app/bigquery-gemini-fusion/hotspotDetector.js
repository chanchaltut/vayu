// bigquery-gemini-fusion/hotspotDetector.js
// Orchestrator: queries BigQuery → groups into geo-clusters → calls Gemini fusion → saves hotspots
// Runs every 15 minutes via Cloud Function (or manually via POST /hotspots/detect)

import { v4 as uuidv4 } from 'uuid'
import { runQuery, DATASET } from '../api/config/bigquery.js'
import { writeHotspot } from '../data-pipeline/bqWriter.js'
import { runFusion } from './fusionEngine.js'
import dotenv from 'dotenv'
dotenv.config()

// ─── Main exported function ────────────────────────────────────────────────────
export const detectAndSaveHotspots = async () => {
  ...
  for (const cluster of clusters) {
    try {
      ...
      if (fusionResult.is_hotspot) {
        ...
        console.log(`[DETECTOR] ✅ Hotspot saved: lat=${cluster.lat}, lon=${cluster.lon}, severity=${fusionResult.severity}/10`)
      } else {
        console.log(`[DETECTOR] ✓ Cluster at lat=${cluster.lat}, lon=${cluster.lon} — no hotspot (severity=${fusionResult.severity})`)
      }
      ...
      console.error(`[DETECTOR] Error processing cluster at lat=${cluster.lat}:`, clusterErr.message)
    }
  }

  console.log(`[DETECTOR] Done. ${hotspotsSaved} hotspots saved out of ${hotspotsDetected} clusters analysed.`)
  return { hotspotsDetected, hotspotsSaved }
}

// ─── Fetch recent sensor readings from BigQuery ────────────────────────────────
const fetchRecentReadings = async (minutes) => {
  const query = `
...
// ─── Run directly (node hotspotDetector.js) ───────────────────────────────────
// Useful for testing: npm run detect-hotspots
