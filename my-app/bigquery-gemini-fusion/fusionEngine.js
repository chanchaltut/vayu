// bigquery-gemini-fusion/fusionEngine.js
// THE CORE AI BRAIN OF VAYU
// Takes recent sensor readings → calls Gemini → returns hotspot decision
// This is the feature that scores the 25% "AI doing real work" criterion

import { geminiPro } from '../api/config/gemini.js'
...
// Output: { is_hotspot, severity, confidence, predicted_aqi_24h, action, reasoning }

export const runFusion = async (clusterReadings, photoAnalysis = null) => {
  if (!clusterReadings || clusterReadings.length === 0) {
    throw new Error('No readings provided to fusion engine')
  }

  // ── Compute aggregates ──────────────────────────────────────────────────
  const arduinoReadings = clusterReadings.filter(r => r.source === 'arduino' && r.temperature !== null)
  const hasTemperature  = arduinoReadings.length > 0

  const avgAqi  = Math.round(clusterReadings.reduce((s, r) => s + r.aqi, 0) / clusterReadings.length)
  const avgTemp = hasTemperature
    ? (arduinoReadings.reduce((s, r) => s + r.temperature, 0) / arduinoReadings.length).toFixed(1)
    : null
  ...
  const sortedByTime = [...clusterReadings].sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp))
...
${JSON.stringify(clusterReadings.slice(0, 8).map(r => ({
  aqi: r.aqi,
  temp: r.temperature,
  source: r.source,
  time: r.timestamp,
...
`
  ...
  console.log(`[FUSION] Result: hotspot=${parsed.is_hotspot}, severity=${parsed.severity}, aqi=${avgAqi}, action="${parsed.action}"`)

  return {
    ...parsed,
    avg_aqi: avgAqi,
    avg_temperature: hasTemperature ? parseFloat(avgTemp) : null,
